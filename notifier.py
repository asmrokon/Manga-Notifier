...
def add_manga():
    ...
    if "myanimelist.net" not in url and "mangaupdates.com" not in url:
        send_in_app_notifications(
            "Error: Please enter a valid site link.", "warning.png"
        )
        return
    if "manga" not in url and "series" not in url:
        send_in_app_notifications(
            "Error: Please enter a valid manga link.", "warning.png"
        )
        return
    ...
    try:
        ...
        send_in_app_notifications(
            "Could not fetch – check your connection.", "warning.png"
        )
        return
    ...
    if "myanimelist.net" in url:
        try:
            ...
            send_in_app_notifications(
                "Could not get manga title. Please check the link.", "warning.png"
            )
            return
